...
class Viewer(Node):
    ...
    def run(self):
        ...
        # 4th param = -(water height) + small overlap to prevent glitches
        reflection_clip_plane = (0.0, 1.0, 0.0, -WATER_HEIGHT+0.5)
        refraction_clip_plane = (
            0.0, -1.0, 0.0, WATER_HEIGHT+0.5)  # = water height
        fog = (1, 0.2, 0.2)
        ...
        while not glfw.window_should_close(self.win):
            ...
            # for reflection/refraction clip planes
            GL.glEnable(GL.GL_CLIP_PLANE0)
            cam_pos = np.linalg.inv(self.trackball.view_matrix())[:, 3]
            # cam_pos - distance (same dist underwater)
            cam_pos[1] = cam_pos[1] - 2*(cam_pos[1] - WATER_HEIGHT)
    ...
    def compute_view_matrix_for_reflection(self, cam_pos):
        ...
        lookAtPosition = cam_pos[:3] + \
            self.trackball.distance * invertedDirection
