...
class BillboardAnimation(Textured):
    ...
    def __init__(self, shader, x, y, z, texturepath, **params):
        ...
        self.vertices = np.zeros(
            (self.number_of_particles, 4, 3), dtype=np.float32)
        self.tex_coords = np.zeros(
            (self.number_of_particles, 4, 2), dtype=np.float32)
        self.indices = np.zeros((self.number_of_particles, 6), dtype=np.uint32)

        for i in range(self.number_of_particles):
            self.indices[i] = [i * 4, i * 4 + 1, i *
                               4 + 2, i * 4 + 2, i * 4 + 1, i * 4 + 3]
        ...
        for i in range(self.number_of_particles):
            ...
            self.vertices[i, 0] = billboard_pos - billboard_right * \
                self.point_size + billboard_up * self.point_size
            self.vertices[i, 1] = billboard_pos + billboard_right * \
                self.point_size + billboard_up * self.point_size
            self.vertices[i, 2] = billboard_pos - billboard_right * \
                self.point_size - billboard_up * self.point_size
            self.vertices[i, 3] = billboard_pos + billboard_right * \
                self.point_size - billboard_up * self.point_size

            self.tex_coords[i] = np.array([[0, 0], [1, 0], [0, 1], [1, 1]])

        attributes = dict(position=self.vertices.reshape(-1, 3),
                          texcoord=self.tex_coords.reshape(-1, 2))
    ...
    def draw(self, primitives=GL.GL_TRIANGLES, attributes=None, **uniforms):
        ...
        # Calculate the model matrix for the current particle
            billboard_up = np.array([0, 1, 0], dtype=np.float32)
            billboard_right = np.cross(
                billboard_up, view_matrix[:3, 0:3].T @ np.array([0, 0, -1], dtype=np.float32))
            ...
            billboard_matrix[:3, :3] = np.column_stack(
                (billboard_right, billboard_up, -billboard_forward))
        ...
        # Calculate the final size and texture coordinates of the particle
            ...
            self.vertices[i, 0] = billboard_pos - \
                billboard_right * size + billboard_up * size
            self.vertices[i, 1] = billboard_pos + \
                billboard_right * size + billboard_up * size
            self.vertices[i, 2] = billboard_pos - \
                billboard_right * size - billboard_up * size
            self.vertices[i, 3] = billboard_pos + \
                billboard_right * size - billboard_up * size
